  ...
  while (!BFS.empty() && !found){
    ...
    if (nextPointX == endX && nextPointY == endY){
      ...
      if (tmpIdx == colorList.size()){
        ...
        nId = getNextId();
        if (data->colorIdx == 0 && nId != numThreads){
          pin = new pthreadInput(nId, tmpIdx, tmpMap);
          rc = pthread_create(&thread[nId], NULL, analyzeDotPair, (void*) pin);
          if (rc == 0){
            ...
            pin->tid = numThreads;
            analyzeDotPair((void*) pin);
          }
        }
        else
          analyzeDotPair((void*) (new pthreadInput(numThreads, tmpIdx, tmpMap)));
